...
// Створює token account і одразу мінтить tokens юзеру
...
// Спільна підготовка: створює mint + пул. Повертає (stake_mint, pool, vault)
fn setup(svm: &mut LiteSVM, authority: &Keypair) -> (Pubkey, Pubkey, Pubkey) {
...
fn test_stake_increases_balances() {
    ...
    let (stake_mint, pool, vault) = setup(&mut svm, &authority);
    let (user_stake, _) = user_stake_pda(&pool, &user.pubkey(), &program_id);
    let user_token_account = create_token_account_with_tokens(
        &mut svm,
        &authority,
        &stake_mint,
        &user.pubkey(),
    ...
    let ix = Instruction::new_with_bytes(
        program_id,
        &staking_protocol::instruction::Stake { amount: 500_000 }.data(),
    ...
    // Перевіряємо що pool.total_staked оновився
    ...
    // Перевіряємо що токени фізично потрапили у vault
    let vault_account = svm.get_account(&vault).unwrap();
    let vault_data = spl_token::state::Account::unpack(&vault_account.data).unwrap();
    assert_eq!(vault_data.amount, 500_000);
}
...
fn test_stake_zero_fails() {
    ...
    let ix = Instruction::new_with_bytes(
        program_id,
        &staking_protocol::instruction::Stake { amount: 0 }.data(),
        staking_protocol::accounts::Stake {
            user: user.pubkey(),
            pool,
            user_stake,
