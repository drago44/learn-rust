use crate::{hashing, hdwallet, keys, mnemonic, signing};
...
pub fn run() {
    ...
    let sol_key0 = hdwallet::derive_sol_keypair(&seed, 0);
    let sol_key1 = hdwallet::derive_sol_keypair(&seed, 1);
    println!(
        "  SOL[0]:   {}",
        keys::solana_address(&sol_key0.verifying_key())
    );
    println!(
        "  SOL[1]:   {}",
        keys::solana_address(&sol_key1.verifying_key())
    );
}
